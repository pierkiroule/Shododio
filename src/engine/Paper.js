...
export const clearPaper = (ctx, width, height) => {
  ...
  for (let i = 0; i < 60000; i += 1) {
    ...
    ctx.fillStyle = `rgba(80,80,75,${shade * 0.08})`;
...
export const resizePaper = ({
  ...
  const previewWidth = ratio >= 1 ? previewLongEdge : Math.round(previewLongEdge * ratio);
  const previewHeight = ratio >= 1 ? Math.round(previewLongEdge / ratio) : previewLongEdge;
